...
contract InterestRateModel is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    /// @dev Lending Pool address => base rate
    ...
    /// @dev Lending Pool address => rate at optimal
    ...
    /// @dev Lending Pool address => optimal utilization
    ...
    /// @dev Lending Pool address => max utilization
    ...
    {
        uint256 interestPerYear = (_totalBorrowAssets(_lendingPool) * _borrowRate) / scaledPercentage; // borrowRate is scaled by 100
    ...
    function _totalSupplyAssets(address _lendingPool) internal view returns (uint256) {
        return ILendingPool(_lendingPool).totalSupplyAssets();
    }
